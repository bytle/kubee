...
Because we are starting from the `simplest Kubernetes cluster` as stated in
the [official documentation](https://kubernetes.io/docs/setup/production-environment/#production-control-plane).
...
* Single node Kubernetes with a goal on low memory consumption. A single node can host 110 containers, that's a lot of
  applications.
* Zero configuration. The [kubee charts](https://github.com/EraldyHq/kubee/blob/main/docs/site/kubee-helmet-chart.md)
  being aware of each other, they are preconfigured and needs a minimal set of parameters.
* Cluster configuration. Every chart configuration is saved in
  a [single cluster values file](https://github.com/EraldyHq/kubee/blob/main/docs/site/cluster-values.md)
...
* Authentication Ready. All infra apps are automatically secured by authentication (`basicAuth` by default, oidc
  if [Dex](https://github.com/EraldyHq/kubee/blob/main/charts/dex/README.md))
* Secure Ready. With the internal `kubee` ca, all communication are encrypted (ssl/https) and the servers get
  certificates that are automatically rotated.
...
* Host, System and App Monitoring
  with [Prometheus](charts/prometheus), [Alert Manager](charts/alertmanager), [Grafana](charts/grafana/README.md), ...
...
* `Secret Management`
  with [Kubernetes Secret](https://kubernetes.io/docs/tasks/configmap-secret/), [External Secret](charts/external-secrets/README.md), [Vault](charts/vault/README.md), ...)
...
you can read
the [kubee chart documentation](https://github.com/EraldyHq/kubee/blob/main/docs/site/kubee-helmet-chart.md).
...
# for the actual dev version
brew install --HEAD bytle/tap/kubee
```

### Getting Started

See [How to create a cluster and install applications](website/src/content/docs/cluster/cluster-creation.md)

## Contribute

See [Contribute/Dev](.github/CONTRIBUTING.md)
